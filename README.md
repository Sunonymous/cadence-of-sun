...
* [`src/cadence_of_sun/`](src/cadence_of_sun/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cadence_of_sun/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `cadence-of-sun` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/cadence_of_sun/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns cadence-of-sun.example
  (:require [cadence-of-sun.config :as config])
